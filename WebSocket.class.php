...
class WebSocket
{
    ...
    public $login=false;
    private $broker=18;  //券商ID
    private $net=0;		//网络类型
    private $sever=2;//交易服务
    ...
    public function __construct($options = array()) {
		...
		$this->socket_uri = 'ws://192.168.1.109:8080?rid='.$uri.'&flag=1';
    }
	...
	**/
    ...
    public function login(){
        $para = array("Broker" => $this->broker,"Net" => $this->net,"Server" => $this->sever,"ClientVer" => $this->clienver);
        $po['req']="Trade_Init";
        $po['rid']="1";
        $po['para']=$para;
        $po=json_encode($po);
        ...
        if($rec1['data']['result']==0){
            if($message['ret']==0){//ret代表请求成功还是失败，0表示成功，非0表示失败
                $data = $message['data'][0];
                $para = array("AccountMode"=> 8,"Broker" => $data['Broker'],"CreditAccount" => 0,"DeptID"=>1,"Encode"=>0,"IP"=>$data['IP'],"LoginID"=>"证券账户","LoginPW"=>"密码","Port"=>$data['Port'],"TradeID"=>"","CommPW"=>"");
                ...
                if($LoginMessage['data']['result']==0){
                    $this->login_tid=$LoginMessage['data']['tid'];
                    $this->login=true;
                }
            }
        }
